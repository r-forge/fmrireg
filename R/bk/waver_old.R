...
makeRegressor <- function(HRF, onsets, amp=NULL, makefun=makeEventHRF) {

  #browser()

  .amp <- amp
  if (is.null(.amp)) {
    .amp <- rep(1, length(onsets))
  }

  funlist <- lapply(1:length(onsets), function(i) makefun(onsets[i], HRF, .amp[i]))
...
sampleRegressor <- function(HRF, onsets, TR, nsamples, amp=NULL) {
  #browser()
  sgrid <- seq(TR/2, by=TR, length.out=nsamples)
  makeRegressor(HRF, onsets,amp=amp)(sgrid)
}

sampleRegressor2 <- function(HRF, onsets, TR, nsamples) {
  sgrid <- seq(TR/2, by=TR, length.out=nsamples)
  makeRegressor(HRF, onsets, makefun=.makeEventHRF3)(sgrid)
}
